...
"""
    ...
    This script will process all given video
"""
import cv2
import time
import numpy as np
...
vid_list = ['demo.train.avi', 'demo.old.avi']
...
if __name__=='__main__':
    ...
    #''' <<<<<<< New version
    #   NO NEED TO RESTORE!
    model = CPM.CPM(pretrained_model='IchigoBrain/model/model.npy', load_pretrained=True, training=False, cpu_only=False,
                    stage=6)
    model.BuildModel()
    #>>>>>>>>>>>>'''
            ...
            try:
                ...
                for n in range(parrallel_num):
                    ...
                    if ret:
                        ...
                        print(ret)
                        break
                j, w = pdt.predict(batch_frame, model=model, debug=False, is_name=False)
                j = np.array(j)
                w = np.array(w)
                t_pred = time.time()
                if j_smooth is None:
                ...
                for n in range(parrallel_num):
                    if j_t is None or w_t is None:
                        ...
                        j_smooth[n, np.where((w_t==1)&(w[n]==1))] = (1-smooth_factor)*j[n, np.where((w_t==1)&(w[n]==1))].astype(np.float) + smooth_factor * j_t[np.where((w_t==1)&(w[n]==1))]
                ...
                t_proj = time.time()
                print("Cost: predict %.2f ms. smoothing %.2f ms projection %.2f ms."%((t_pred - t1)*1000, (t_smos-t_pred)*1000, (t_proj-t_smos)*1000))
                '''
                ...
                for n in range(parrallel_num):
                    ...
                    if first == True:
                        ...
                        out = cv2.VideoWriter('out_'+str(smooth_factor*10)+'_'+vid_name, fourcc, 30, ret_img_s.shape[:-1][::-1])
                        first = False
                    if out is not None:
                        out.write(ret_img_s)
